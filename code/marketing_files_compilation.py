...
def transform_dataframes(name_list: list, breakdown: str):
  ...
  for i in range(len(name_list)):
    ...
    if (i>0) & ('events' in name_list[i]):
        complete_dataframe = pd.merge(complete_dataframe, a, how = 'left', on = ['date', 'campaign_id', 'adset_id'])
    else:
...
if __name__ == '__main__':
    ...
    try:
        body = f.read()
        print("Your key is: ", str(sys.argv[1]))
        transform_dataframes(name_list = dict_list[str(sys.argv[1])], breakdown = translate_break[str(sys.argv[1])])
